...
WHERE CASE WHEN '{{var('site')}}' IN ('ALL', '') THEN site_name ELSE '{{var('site')}}' END LIKE CONCAT ('%', site_name, '%')
 AND CASE WHEN '{{var('system')}}' IN ('ALL', '') THEN source_system_name ELSE '{{var('system')}}' END LIKE CONCAT ('%', source_system_name, '%')  " if is_incremental() else "",
        unique_key = ['site_code', 'system_code', 'hole_id', 'hole_name', 'block']
    )
}}

WITH hole_data AS (
  select site_name,source_system_name,hole_id,hole_name,block 
...
SELECT 
  ...
  hd.block,
  hd.site_name, 
  hd.source_system_name,
  {{add_tech_columns(this)}}
FROM hole_data AS hd
JOIN group_aadp_l3_l4_{{var('env')}}.staging_l3.man_site_lookup AS sl
ON lower(hd.site_name) = lower(sl.source_site_code) 
    AND lower(hd.source_system_name) = lower(sl.source_system_name)
