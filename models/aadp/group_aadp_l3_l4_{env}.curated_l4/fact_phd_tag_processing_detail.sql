...
WHERE EXISTS (
    ...
    WHERE fact.site_sk = site.site_sk
      ...
      AND dd.DATE >= date_sub(CURRENT_DATE (), {{var('refresh_days')}})
        ...
        unique_key = ['system_sk', 'site_sk', 'site_reporting_group_sk', 'date_sk', 'reporting_period_sk', 'site_system_shift_date_sk', 'asset_sk', 'process_sk', 'material_sk', 'scenario_sk', 'functional_location_sk', 'department_sk', 'received_dispatched_site_reporting_group_sk', 'data_indicator_sk', 'tag_code', 'dictionary', 'metric_code', 'reading_date_time_utc']
    )
}}

WITH cte_fact_phd_tag_processing_detail AS (
SELECT COALESCE(sys.system_sk,-1) AS system_sk,
       ...
       CASE WHEN UPPER(l3.aggregation_type) IN ('','SUM','NO GROUPING') THEN SUM(l3.amount)
            ...
            --begin 'LAG(>BOOLEAN)' MUST FOLLOW BELOW ORDER FOR the FALSE STATEMENT TO WORK
            WHEN UPPER(l3.aggregation_type) IN ('LAG(>BOOLEAN)') and max(l3.amount) > LAG(max(l3.amount)) OVER(PARTITION BY l3.tag_code ORDER BY l3.reading_date_time_utc) then 1 
            WHEN UPPER(l3.aggregation_type) IN ('LAG(>BOOLEAN)') THEN 0
            --end 'LAG(>BOOLEAN)'
            ELSE SUM(l3.amount)
       END AS amount,
       l3.unit_of_measure_code,
       uom.name AS unit_of_measure_name,
       l3.reading_date_time_utc AS reading_date_time_utc,
       l3.reading_date_time_local AS reading_date_time_local,
 ...
 WHERE 1=1
   AND UPPER(l3.time_scale) NOT IN ('SHIFT')
...
SELECT l3.system_sk, l3.site_sk, l3.site_reporting_group_sk, l3.date_sk, l3.reporting_period_sk, l3.site_system_shift_date_sk, l3.asset_sk,
       ...
       l3.reading_date_time_utc, l3.reading_date_time_local, l3.processing_sequence, l3.time_scale, l3.is_active, l3.exception_flag,
       {{add_tech_columns(this)}}
  FROM cte_fact_phd_tag_processing_detail AS l3
