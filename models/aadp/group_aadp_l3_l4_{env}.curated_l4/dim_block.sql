...
{{
    config(
        ...
        schema='curated_l4',
        matched_condition = generate_matched_condition(['site_code', 'site_name', 'system_code', 'source_system_name', 'block']) ,
        tags = ['incremental'],
        target_alias = "tgt",
        source_alias = "src", 
        meta =  { 
...
select --row_number() over(order by site_name, source_system_name,block) as block_sk,
(100000000000 * cast(site.site_sk AS BIGINT) + 1000000000 * cast(sys.system_sk AS BIGINT))  + crc32(block) AS block_sk
,A.site_code
,A.site_name
,A.system_code
