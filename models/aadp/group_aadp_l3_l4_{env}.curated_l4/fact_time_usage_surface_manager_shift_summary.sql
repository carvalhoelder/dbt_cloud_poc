...
WHERE EXISTS (
    ...
    WHERE fact.site_sk = site.site_sk
      ...
      AND dd.DATE >= date_sub(CURRENT_DATE (), {{var('refresh_days')}})
        ...
        unique_key = ['system_sk', 'site_sk', 'site_reporting_group_sk', 'date_sk', 'asset_sk', 'process_sk', 'scenario_sk', 'site_system_shift_date_sk', 'department_sk', 'reporting_period_sk', 'functional_location_sk']
    )
}}

WITH SURF_MNGR_KEYS_CTE
AS (
  ...
  FROM {{ ref('surface_manager_tum') }} AS summary
...
SURF_MNGR_L4_CTE AS (
  ...
  FROM SURF_MNGR_KEYS_CTE
  GROUP BY sk, date_sk
  ),
SURF_MNGR_DISTINCT_KEYS_CTE
AS (
  ...
  FROM SURF_MNGR_KEYS_CTE AS keys_cte
  )
...
FROM SURF_MNGR_DISTINCT_KEYS_CTE AS keys_cte
INNER JOIN SURF_MNGR_L4_CTE AS l4_cte
